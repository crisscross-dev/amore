...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const scheduleConflictMessage = document.getElementById('scheduleConflictMessage');
  ...
  document.querySelectorAll('form[action*="/admin/sections/"][method="post"]').forEach((form) => {
  ...
  if (selectAllCheckbox && studentCheckboxes.length > 0) {
    ...
    if (bulkForm) {
      bulkForm.addEventListener('submit', function(e) {
        ...
        const confirmed = confirm(`Add ${checkedCount} student${checkedCount > 1 ? 's' : ''} to this section?`);
        if (!confirmed) {
          e.preventDefault();
        }
      });
  ...
  if (studentSearch) {
    console.log('studentSearch found, studentItems count:', studentItems.length);
    ...
    console.log(`Loaded ${availableStudentsData.length} available students`);
    ...
    function renderStudentList(students) {
      ...
      searchSuggestions.innerHTML = students.map(student => {
        const isChecked = selectedStudents.find(s => s.id === student.id) ? 'checked' : '';
      ...
      searchSuggestions.querySelectorAll('.student-select-checkbox').forEach(checkbox => {
    ...
    function updateSelectedDisplay() {
      if (selectedStudents.length > 0) {
        ...
        // Add remove handlers
        selectedStudentsList.querySelectorAll('.remove-selected-btn').forEach(btn => {
          btn.addEventListener('click', function() {
            const studentId = this.dataset.studentId;
            selectedStudents = selectedStudents.filter(s => s.id !== studentId);
            
            // Uncheck the checkbox
            const checkbox = document.querySelector(`.student-select-checkbox[data-student-id="${studentId}"]`);
    ...
    if (confirmAddBtn) {
      confirmAddBtn.addEventListener('click', function() {
        ...
        const confirmed = confirm(`Add ${selectedStudents.length} student${selectedStudents.length > 1 ? 's' : ''} to this section?`);
        if (!confirmed) return;
